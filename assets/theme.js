...
(function ($) {
   ...
   */
        ...
        return $(this)
            ...
            .each(function () {
                ...
                var $cont = $(this)
                    ...
                    .append('<span class="text"><span class="value">' + initialText + '</span></span>' + chevronDownIcon); //Label? Move inside
    ...
    theme.VideoManager = new (function () {
        ...
        _._loadMp4Videos = function (container) {
            if ($(_.mp4Vars.toProcessSelector, container).length) {
                // play those videos
                $(_.mp4Vars.toProcessSelector, container)
                    .addClass('video--init')
                    .each(function () {
    ...
    theme.ProductMediaGallery = function ($gallery, $thumbs, isFeaturedProduct, isQuickbuy, galleryId) {
        ...
        function handleImageClick() {
            ...
            if (theme.viewport.isSm()) {
                ...
                showThemeModal(
                    $('<div class="theme-modal theme-modal--fullscreen temp -light"  role="dialog" aria-modal="true"/>').append(
                        "\n           <div class='inner-scroller -out'>".concat(imageHtml, '</div>')
                    ),
                    'product-image',
                    function ($modal) {
        ...
        $(window)
            .off('ccScrollToMedia.productTemplateGallery'.concat(galleryId))
            .on('ccScrollToMedia.productTemplateGallery'.concat(galleryId), function (e, mediaId) {
                ...
                .slick({
                    ...
                    customPaging: function customPaging(slider, i) {
                        return (
                            ...
                            '<circle class="circle-two" cx="15" cy="15" r="13" style="animation-duration: '.concat(autoplaySpeed + speed, 'ms" />') +
                            '</svg>' +
                            '</button>'
                        );
                    },
        ...
        if ($clickies.length > 0) {
            ...
            for (var optionIndex = 0; optionIndex < productData.options.length; optionIndex++) {
                ...
                for (var key in optionValues) {
                    if (!optionValues[key]) {
                        $('.selector-wrapper:eq(' + optionIndex + ') .clickyboxes li a', container)
                            .filter(function () {
    ...
    theme.loadInfiniteScroll = function (container) {
        ...
        $('[data-infinite-scroll-container] .pagination.infiniscroll:not(.infinit)', container)
            .addClass('infinit')
            .each(function () {
                        ...
                        if ($form.hasClass('feedback-add_in_modal') || $form.hasClass('feedback-add_in_modal_no_checkout')) {
                            ...
                            showThemeModal(
                                [
                                    ...
                                    '<div class="cart-product__image"><img src="'.concat(thumbUrl, '" alt="').concat(product.featured_image.alt, '"/></div>'),
                                    '<div class="cart-product__content">' + '<p class="cart-product__content__title">',
                                    product.product_title,
                                    '</p>' + ''.concat(variantHtml ? variantHtml : '') + '</div>',
                                    '</div>',
                                    '<p class="links '.concat(noCheckoutButton ? 'links--no-checkout' : '', '">'),
        ...
        _.updateInventoryNotice = function (variant, $container) {
            ...
            if (inventoryData) {
                $container
                    ...
                    .html(_._getString('inventoryNotice').replace('[[ quantity ]]', inventoryData))
                    .slideDown(300);
            } else {
        ...
        this.onSectionLoad = function (target) {
            ...
            function resizeProductDetails() {
                if (theme.viewport.isSm()) {
                    ...
                    if ($('body').hasClass('header-section-overlap2') && !nav.bar.isInline()) {
                            ...
                            $($ajaxContainers[index])
                                ...
                                .each(function () {
                                    ...
                                    if ($(this).hasClass('is-open')) {
                                        $newAjaxContainer
                                            .find('.cc-accordion:nth-child('.concat(accordionIndex + 1, ') .cc-accordion-item'))
                                            .addClass('is-open')
                                            .attr('open', '');
                                    } else {
                                        $newAjaxContainer
    ...
    jQuery(function ($) {
            ...
            if (theme.strings.back.length > 0) {
                $menuList.prepend(
                    '<li class="main-nav__back" data-nav-title="'
                        .concat(trimmedTitle, '"><a href="#" data-revert-back><span class="arr arr--left">')
        ...
        $(document).ready(function () {
            $('.cc-accordion-item__title').each(function () {
                $this = $(this);
